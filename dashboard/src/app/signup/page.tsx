    ...
    <main className="min-h-screen flex items-center justify-center p-4 bg-navy-900">
      ...
      <div className="relative w-full max-w-md animate-slide-up">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-400 mt-2 font-medium tracking-wide uppercase">Create your account</p>
        </div>
        <div className="glass-card p-8 md:p-10 animate-fade">
          <h2 className="text-xl font-semibold text-slate-100 mb-1">Get started</h2>
          <p className="text-sm text-slate-400 mb-6">Deploy your first database in minutes</p>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="name" type="text" autoComplete="name" placeholder="Alex Johnson" value={name}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="email" type="email" autoComplete="email" placeholder="alex@company.com" value={email}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="password" type={showPassword ? "text" : "password"} autoComplete="new-password"
                  placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} disabled={loading}
            ...
            <div className="flex items-start gap-3 pt-1">
              <input id="terms" type="checkbox" checked={agreedToTerms}
                onChange={(e) => setAgreedToTerms(e.target.checked)} disabled={loading}
              ...
              <label htmlFor="terms" className="text-xs text-slate-400 leading-relaxed cursor-pointer select-none">
                I agree to the{" "}
                <span className="text-purple-400 hover:text-purple-300 underline underline-offset-2 transition-colors">Terms of Service</span>{" "}
                and{" "}
                <span className="text-purple-400 hover:text-purple-300 underline underline-offset-2 transition-colors">Privacy Policy</span>
              </label>
            </div>
            {/* Submit */}
          ...
          <p className="mt-6 text-center text-sm text-slate-500">
            ...
            <Link href="/login" className="text-purple-400 hover:text-purple-300 underline underline-offset-4 transition-colors font-medium">Sign in</Link>
          </p>
        </div>
...
function Req({ met, label }: { met: boolean; label: string }) {
  return (
    <div className="flex items-center gap-2 text-xs">
      {met ? <Check size={14} className="text-green-400 shrink-0" /> : <X size={14} className="text-slate-600 shrink-0" />}
      <span className={cn("transition-colors", met ? "text-green-300" : "text-slate-500")}>{label}</span>
    </div>
  );
}
