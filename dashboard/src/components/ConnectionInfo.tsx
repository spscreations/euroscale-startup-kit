...
function CopyButton({ value, label }: { value: string; label: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="rounded-md p-1.5 text-text-muted transition-all hover:bg-navy-600 hover:text-purple-400"
...
function SslCaSection({ pem }: { pem: string }) {
  ...
  return (
    <div className="rounded-lg bg-navy-800/60">
      <button
        onClick={() => setExpanded(!expanded)}
        className="flex w-full items-center justify-between px-4 py-3 text-left transition-colors hover:bg-navy-700/40"
      >
      ...
      {expanded && (
        <pre className="max-h-48 overflow-auto border-t border-glass-border px-4 py-3 font-mono text-xs text-text-secondary">
          {pem}
        </pre>
      )}
    </div>
...
export default function ConnectionInfo({
  ...
  // Determine which connection details to display
  const connString =
  ...
  return (
    <div className="glass-card animate-slide-up">
      ...
      <div className="px-6 pt-4">
        <div className="flex items-center justify-between rounded-lg border border-glass-border bg-navy-800/80 px-4 py-3">
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium uppercase tracking-wider text-text-muted">
              Connection String
            </p>
            <p className="mt-0.5 truncate font-mono text-sm text-cyan-400">
              {connString}
            </p>
          </div>
      ...
      {!hasCredentials && (
        <div className="border-t border-glass-border px-6 py-3">
          <p className="flex items-center gap-2 text-xs text-gold-400">
            <Lock size={12} />
            Credentials are only shown once after creation. Click{" "}
            <strong>&ldquo;Rotate Credentials&rdquo;</strong> to generate new ones.
          </p>
        </div>
      )}
    </div>
